...
// Has direct text/media/mask of its own, or wraps an icon. Excludes <a>/<button>
// from the descendant check — those are interactive targets, not icon containers.
...
// Elements targeted by :hover / :active / :focus* CSS rules. Built lazily,
// reset per picker session.
...
// Returns the compound the state pseudo is attached to, with the pseudo
// stripped. "foo:hover .bar" → "foo". ".ctas a:active .x" → "a".
  ...
  function visit(rules: CSSRuleList): void {
    for (const rule of rules) {
      if (rule instanceof CSSStyleRule) {
        ...
        for (const part of splitTopLevelCommas(rule.selectorText)) {
...
// Resolve a picker hit to the "interactive unit". Keep content-bearing hits
// (text/icon). Otherwise prefer the outermost <a>/<button> or custom-element
// ancestor in the styled cache; fall back to the outermost <a>/<button>,
// optionally promoted one level to a custom-element wrapper.
