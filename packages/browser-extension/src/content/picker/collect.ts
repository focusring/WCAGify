...
// visual values and that the picker often can't land on directly. Structural/landmark/text roles (heading, paragraph,
// listitem, region, table…) are intentionally excluded to avoid drowning a content-rich component in sections.
...
// The role to surface for a descendant, or '' to skip it. Reuses getAriaRole, plus a special case for <svg> (which has
// no implicit ARIA role in our mapping but is meaningful media to inspect — treated as img).
function surfaceableRole(el: Element): string {
  if (el.localName === 'svg') return 'img'
...
// Surfaceable descendants of el, in document order, capped. Excludes el itself (it's the selected section). Includes
// disabled/aria-hidden elements on purpose — surfacing the otherwise-unreachable ones is the point of this feature.
