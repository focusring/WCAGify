...
The inline feedback widget **"<span lang="nl">Wat vind je van deze informatie?</span>"** is a separate document in a frame. That document declares `<html lang="en">` and its title is "Customer feedback element", while the text a user must understand is Dutch: the question itself, "Verplicht", and the landmark name "<span lang="nl">Klantfeedback element</span>". A screen reader follows the declared language, so it reads the Dutch question with English pronunciation rules. The answer names ("thumbs up") are English too, which mixes both languages in one small widget.
