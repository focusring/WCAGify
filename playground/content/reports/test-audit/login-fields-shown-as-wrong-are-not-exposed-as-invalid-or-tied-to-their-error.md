...
When a login field fails its check on leaving it, the page draws it as wrong: an orange border on the e.dentifier boxes, a warning icon and a message below ("Vul minimaal 9 cijfers in.", "Vul minimaal 5 cijfers in.", "Dit veld is verplicht."). None of that reaches assistive technology. `#account-number`, `#card-number`, `#ed2response` and `#email` never get `aria-invalid`, and none has an `aria-describedby` or `aria-errormessage` pointing at its message. The tree reads `textbox "ABN AMRO Rekeningnummer" [required]: 123`.
