...
On the "<span lang="nl">Klant worden</span>" page, the third card under **"<span lang="nl">Daarom een bankrekening bij ABN AMRO</span>"**, "<span lang="nl">Veilig bankieren</span>", has a lock icon (`pr-lock-closed-original.svg`) with `alt='" "'`: two straight quotation marks around a space. That is not an empty alt, so the icon stays in the accessibility tree as an image named `" "`, between the heading "<span lang="nl">Veilig bankieren</span>" and the sentence "<span lang="nl">Altijd veilig bankieren. …</span>".
