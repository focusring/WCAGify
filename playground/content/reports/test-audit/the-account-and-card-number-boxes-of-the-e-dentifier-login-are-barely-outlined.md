...
In the e.dentifier login the two boxes under **"Rekening- en pasnummer"** (`input#account-number` and `input#card-number`) are outlined with a 1 px border in `#cccccc` on the white card: **1.61:1**, where 3:1 is needed. The grey prefix "NL ** ABNA 0" inside the account box (`#f2f7f7`, 1.07:1) does not mark the field either.
