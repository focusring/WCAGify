...
The app page shows two store badges twice, in the hero and in the "Al 3,7 miljoen klanten hebben de ABN AMRO app" block. The badges themselves are Dutch ("Download in de App Store", "ONTDEK HET OP Google Play"), but their `alt` texts are English: **"Download on the App Store"** and **"Download on Google Play"**. No `lang` attribute marks them, so the page's Dutch applies.
...
The text alternative also does not match what the badge shows: "Download on Google Play" is not what the image says ("Ontdek het op Google Play"), so a speech-input user who reads the badge aloud may not reach the link.
