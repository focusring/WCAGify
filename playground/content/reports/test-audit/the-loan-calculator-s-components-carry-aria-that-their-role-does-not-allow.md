...
- In the mortgage wizard (page-31), which uses the same components: the result view's `div.list-items-wrapper` carries `aria-label="Maandlasten"` with no role, so the name that groups the monthly costs is dropped.
