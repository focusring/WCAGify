...
The inline feedback widget **"<span lang="nl">Wat vind je van deze informatie?</span>"** offers two answers, a thumbs-up and a thumbs-down drawing. Both images have `alt="thumbs up"` (the second one is `<img id="thumbs_down" alt="thumbs up" aria-label="thumbs up">`), and each radio button takes its name from its image. A screen reader reads the group as `radio "thumbs up"`, `radio "thumbs up"`. Blind users hear two identical answers and cannot give the negative answer on purpose. The name of the second radio also misstates its value, so this fails 4.1.2 for the same reason.
