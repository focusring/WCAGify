...
"Inloggen met security key" asks for the user's own e-mail address (`input#email`, label "E-mailadres", inside `aab-inputblock-email`), but the field has **no `autocomplete` attribute**. `type="email"` alone does not identify the purpose: it names a format, not whose address it is.
