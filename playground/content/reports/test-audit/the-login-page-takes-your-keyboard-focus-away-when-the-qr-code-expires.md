...
The QR code on the login page expires after a short time — measured three times at **85, 94 and 109 seconds**. When it does, the page replaces the code with a button reading "QR-code verlopen. Klik om een nieuwe QR-code te maken" and **calls `focus()` on that button**, wherever the user happens to be.
