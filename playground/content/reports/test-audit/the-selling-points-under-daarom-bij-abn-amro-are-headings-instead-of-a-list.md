...
Under the heading "Daarom bij ABN AMRO" on the account-opening page, the eye reads three short selling points side by side: **"Altijd en overal online"**, "Jij bepaalt hoe je betaalt" and "Al jouw geld op één veilige plek". In the markup each point is an `h3` inside its own `role="region"` card, and the card's content container is empty.
...
The same pattern is in the "Jij bepaalt hoe je betaalt" block on the payment-account page (page-13): the three checkmark lines "Snel contactloos betalen", "Met Google Pay, Apple Pay of wearable" and "Tot € 50 zonder pincode" are `h3` elements with no content after them and no list around them.
