...
In the desktop layout of the section widget ("<span lang="nl">Informatie over je hypotheek</span>" on this page), choosing an item shows its panel beside the list of items. The panel is not the next Tab stop after the item that opened it: focus first walks the remaining item buttons and only then enters the panel.

Enter on **"<span lang="nl">Actuele hypotheekrente</span>"**, then Tab: focus goes to "<span lang="nl">Korting op de hypotheekrente</span>", then "<span lang="nl">Duurzaam wonen</span>", and only then to "<span lang="nl">Bekijk de hypotheekrente</span>" in the panel. On page load the same widget does the opposite (item 1, its panel's links, then item 2), so the order a keyboard user learns first is not the one they get after a choice. For a screen-reader user who has just opened an item, the content that opened is behind every other item.

The same order is on page-13 (the step foldouts), page-18 ("<span lang="nl">Verder goed om te weten</span>"), page-19 ("<span lang="nl">Meer informatie</span>"), page-20 ("<span lang="nl">Wanneer kun je lenen?</span>") and page-22 (the address-change sections). In this state the widget also traps focus once it reaches the panel, which is a separate issue, "Keyboard focus gets stuck inside the expandable section panels"; the wrong state flags are "A panel widget announces itself as collapsed while its content is on screen".
