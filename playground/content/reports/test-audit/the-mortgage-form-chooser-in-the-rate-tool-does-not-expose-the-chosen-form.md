...
The **"<span lang="nl">Kies een hypotheekvorm</span>"** chooser in the mortgage-rate tool (`div#mortgage_type_selector`, `role="button"`, with the list `ul.em-dropdown__list[role=listbox]`) exposes its list and its value incorrectly in three ways:
...
Keep `aria-selected="true"` on the chosen option, including the forms under "<span lang="nl">Toon alle vormen</span>". Point `aria-activedescendant` only at ids that exist, or leave it out while DOM focus moves onto the options, as it does now ([ARIA5](https://www.w3.org/WAI/WCAG22/Techniques/aria/ARIA5), [APG select-only combobox](https://www.w3.org/WAI/ARIA/apg/patterns/combobox/examples/combobox-select-only/)).
